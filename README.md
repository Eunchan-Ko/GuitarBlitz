...
css/style.css       Tailwind 로 표현 못 하는 커스텀 스타일 (지판, 애니메이션)
js/config.js        음이름·표준 튜닝·엔진 파라미터 등 상수
...
js/game.js          출제 / 타이머 / 정답·오답 판정
js/main.js          진입점, DOM 이벤트 바인딩
```

- 스크립트는 `index.html` 하단에서 위 순서대로 로드됩니다 (`config` → `state` → 엔진 → `game` → `main`).
- ES 모듈을 쓰지 않으므로 `index.html` 을 더블클릭해 `file://` 로 열어도 동작합니다.
- 튜닝·판정 기준·지판 레이아웃 값은 `js/config.js` 한 곳에서 조정합니다.
